# nRF Cloud CoAP Onboarding

The 91m1 application connects to nRF Cloud over **CoAP/DTLS** from the host MCU (nRF54L15). Cellular data goes through the nRF91M1 Serial Modem via PPP; credentials are stored on the host using the TLS credentials shell and TF-M Protected Storage.

## Overview

Before the device can authenticate to nRF Cloud CoAP, you must:

1. Create a self-signed CA certificate.
2. Generate and install device credentials on the host (CA cert + private key for JWT signing).
3. Onboard the device to your nRF Cloud account (register the device public key).

Until onboarding is complete, `nrf_cloud_coap_connect()` will fail even if credentials are installed locally.

## Prerequisites

1. Complete [hardware setup](hardware-setup.md) and flash Serial Modem firmware with PPP enabled on the nRF9151 DK.
...
3. Open a serial terminal on the **nRF54L15 console** (uart20 — the DK USB port used for the host shell, not the modem link on uart30).
4. Confirm the application boots and prints network/cloud logs. Note the **Device ID** from the boot log:

   ```text
   <inf> nrf_cloud_info: Device ID: E4361AD8FD3E4554
   ```

   This 16-character hex ID comes from the nRF54L15 SoC (`CONFIG_NRF_CLOUD_CLIENT_ID_SRC_HW_ID`). Use it as the device ID for credential generation and portal onboarding. It is **not** the nRF91 modem UUID.

## Install nRF Cloud Utils

Install the Python tools used for CA creation, credential installation, and portal onboarding:
...
Create a CA certificate that will sign device credentials. You only need to do this once; reuse the same CA for all devices.
...
This produces three files (serial number varies):

- `self_<serial>_ca.pem` — CA certificate
- `self_<serial>_prv.pem` — CA private key
- `self_<serial>_pub.pem` — CA public key

Keep the CA private key secure. You will use the CA cert and key in the next step for each device.

## Onboard a device

Complete these steps for each host you want to connect to nRF Cloud.

### 1. Flash firmware and read the device ID

With 91m1 running, copy the **Device ID** from the serial log (see [Prerequisites](#prerequisites)). The cloud module logs it again when connecting:

```text
<inf> cloud: nRF Cloud client ID: E4361AD8FD3E4554
```

### 2. Install credentials with `device_credentials_installer`

Run the installer from the directory containing your self-signed CA files. Point it at the nRF54L15 serial port and pass the device ID from the boot log.
...
Delete the device private key PEM from your host after installation if you used `--local-cert`.

The default nRF Cloud security tag is **16842753** (`CONFIG_NRF_CLOUD_SEC_TAG`).

### 3. Onboard the device to your nRF Cloud account

Register the device public key with nRF Cloud using the generated CSV:
