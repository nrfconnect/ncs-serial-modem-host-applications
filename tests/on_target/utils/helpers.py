...
NRF_CLOUD_DEVICE = REPO_ROOT / ".github/test/cloud/nrf-cloud-device.sh"
SUMMARY_FILE = REPO_ROOT / "build" / "hardware-test-summary.txt"
...
def summary_line(line: str) -> None:
...
def load_expected_device_id(test_config: dict) -> str:
    ...
    try:
        ...
        raise RuntimeError(
            f"{device_id_var} must be set to the nRF54L15 DUT device ID allowlist "
    ...
    if not DUT_DEVICE_ID_RE.fullmatch(normalized):
        raise ValueError(
            f"{device_id_var} must be a 16-character hex nRF54L15 host device ID, "
            f"got {device_id!r}"
        )
    return normalized
...
def run_nrf_cloud_device(*args: str) -> None:
    subprocess.run([str(NRF_CLOUD_DEVICE), *args], check=True, env=os.environ.copy())
