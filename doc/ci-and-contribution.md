...
The Serial Modem console runs at **1000000 baud**, not the usual 115200 — it is sized for modem traces. `console_baudrate` is recorded in [`tests/on_target/ci/serial_modem_firmware.yml`](../tests/on_target/ci/serial_modem_firmware.yml); re-check it when upstream changes the overlay, because a baud mismatch produces an empty log rather than an error.

Modem capture also requires **VCOM1 enabled** in Board Configurator on the nRF9151 / SMA DK, which is the documented [hardware setup](../applications/91m1_ppp/doc/hardware-setup.md). The same setup disconnects VCOM0 and VCOM0 HWFC, because the nrf91m1 build puts the host AT link on `uart0` — the pins VCOM0 otherwise occupies. A rig that skips either switch fails with `init_chat_script: timed out`. The port is resolved from the rig's `CI_*_SERIAL_MODEM_SEGGER_SN`; set the matching `CI_*_SERIAL_MODEM_SERIAL_PORT` variable to pin it explicitly.
